...
@section('content')
    ...
    <div style="margin: 2%;" class="listCliente">
        <form action="{{ route('vendas_invalidas', auth()->user()->id) }}" class="position-relative col-md-6">
            ...
            <ul class="list-group" style="cursor: pointer;">
                @foreach ($carrinhos as $key_cliente => $carrinho)
                    @foreach ($carrinho as $c)
                        <li onclick='modalItensVendaInvalida(<?php echo $c; ?>,<?php echo json_encode($produtos); ?> )'
...
@endsection
<script type="text/javascript" src="{{ asset('js/vendaInvalidas.js') }}" defer></script>
